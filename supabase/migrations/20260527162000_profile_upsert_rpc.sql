-- Robust profile creation/claiming for Supabase Auth users.
-- The mobile client calls this after authentication succeeds, so profile writes
-- do not depend on client-side RLS edge cases or legacy email-linking rows.
